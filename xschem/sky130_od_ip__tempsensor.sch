v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
N -640 -260 -430 -260 {
lab=vdd}
N -630 -650 -590 -650 {
lab=vdd}
N -540 -290 -540 -260 {
lab=vdd}
N -560 -290 -540 -290 {
lab=vdd}
N -640 -30 -640 -20 {
lab=vss}
N -640 -20 -430 -20 {
lab=vss}
N -430 -30 -430 -20 {
lab=vss}
N -530 -20 -530 -0 {
lab=vss}
N -550 0 -530 0 {
lab=vss}
N -630 -610 -590 -610 {
lab=vss}
N -430 -200 -430 -90 {
lab=#net1}
N -640 -200 -640 -180 {
lab=#net2}
N -640 -180 -640 -170 {
lab=#net2}
N -640 -110 -640 -90 {
lab=#net3}
N -430 -260 -280 -260 {
lab=vdd}
N -270 -180 -120 -180 {
lab=#net4}
N -220 -200 -220 -180 {
lab=#net4}
N -280 -260 -220 -260 {
lab=vdd}
N -430 -20 -120 -20 {
lab=vss}
N -120 -120 -120 -80 {
lab=#net5}
N -270 -120 -270 -80 {
lab=#net6}
N -220 -260 120 -260 {
lab=vdd}
N -120 -20 120 -20 {
lab=vss}
N 120 -200 120 -180 {
lab=#net7}
N 120 -120 120 -80 {
lab=#net8}
N 30 -150 80 -150 {
lab=#net7}
N 60 -190 120 -190 {
lab=#net7}
N 60 -190 60 -150 {
lab=#net7}
N 80 -100 80 -50 {
lab=#net8}
N 80 -100 120 -100 {
lab=#net8}
N -10 -120 -10 -80 {
lab=#net9}
N -10 -200 -10 -180 {
lab=Vp}
N -10 -190 50 -190 {
lab=Vp}
N -600 -220 210 -220 {
lab=Vp}
N -600 -230 -600 -220 {
lab=Vp}
N -470 -230 -470 -220 {
lab=Vp}
N -180 -230 -180 -220 {
lab=Vp}
N 30 -230 30 -220 {
lab=Vp}
N 80 -230 80 -220 {
lab=Vp}
N 50 -220 50 -190 {
lab=Vp}
N -650 -230 -640 -230 {
lab=vdd}
N -650 -260 -650 -230 {
lab=vdd}
N -650 -260 -640 -260 {
lab=vdd}
N -430 -230 -420 -230 {
lab=vdd}
N -420 -260 -420 -230 {
lab=vdd}
N -230 -230 -220 -230 {
lab=vdd}
N -230 -260 -230 -230 {
lab=vdd}
N -20 -230 -10 -230 {
lab=vdd}
N -20 -260 -20 -230 {
lab=vdd}
N 120 -230 130 -230 {
lab=vdd}
N 130 -260 130 -230 {
lab=vdd}
N 120 -260 130 -260 {
lab=vdd}
N -290 -50 -270 -50 {
lab=vss}
N -290 -50 -290 -20 {
lab=vss}
N -120 -50 -110 -50 {
lab=vss}
N -110 -50 -110 -20 {
lab=vss}
N -10 -50 10 -50 {
lab=vss}
N 10 -50 10 -20 {
lab=vss}
N 120 -50 130 -50 {
lab=vss}
N 130 -50 130 -20 {
lab=vss}
N 120 -20 130 -20 {
lab=vss}
N -260 -610 -130 -610 {
lab=vdd}
N -260 -550 -260 -490 {
lab=Vbe1}
N -130 -550 -130 -490 {
lab=Vbe2}
N -220 -460 -170 -460 {
lab=vss}
N -260 -430 -260 -410 {
lab=vss}
N -260 -410 -130 -410 {
lab=vss}
N -130 -430 -130 -410 {
lab=vss}
N -190 -410 -190 -380 {
lab=vss}
N -210 -380 -190 -380 {
lab=vss}
N -190 -640 -190 -610 {
lab=vdd}
N -220 -640 -190 -640 {
lab=vdd}
N -170 -580 -170 -570 {
lab=Vp}
N -220 -580 -220 -570 {
lab=Vp}
N -220 -570 -30 -570 {
lab=Vp}
N -130 -580 -120 -580 {
lab=vdd}
N -120 -610 -120 -580 {
lab=vdd}
N -130 -610 -120 -610 {
lab=vdd}
N -270 -580 -260 -580 {
lab=vdd}
N -270 -610 -270 -580 {
lab=vdd}
N -270 -610 -260 -610 {
lab=vdd}
N -190 -460 -190 -410 {
lab=vss}
N -300 -520 -260 -520 {
lab=Vbe1}
N -130 -520 -80 -520 {
lab=Vbe2}
N -620 -500 -570 -500 {
lab=ena}
N -530 -530 -480 -530 {
lab=vdd}
N -530 -500 -510 -500 {
lab=vdd}
N -510 -530 -510 -500 {
lab=vdd}
N -530 -470 -530 -450 {
lab=Vp}
N -530 -450 -480 -450 {
lab=Vp}
N -270 -150 -260 -150 {
lab=#net4}
N -260 -180 -260 -150 {
lab=#net4}
N -130 -150 -120 -150 {
lab=#net4}
N -130 -180 -130 -150 {
lab=#net4}
N -20 -150 -10 -150 {
lab=#net9}
N -20 -150 -20 -120 {
lab=#net9}
N -20 -120 -10 -120 {
lab=#net9}
N 120 -150 130 -150 {
lab=#net8}
N 130 -150 130 -120 {
lab=#net8}
N 120 -120 130 -120 {
lab=#net8}
N -230 -90 -230 -50 {
lab=#net6}
N -270 -90 -230 -90 {
lab=#net6}
N -160 -90 -160 -50 {
lab=#net5}
N -160 -90 -120 -90 {
lab=#net5}
N -120 -90 -50 -90 {
lab=#net5}
N -50 -90 -50 -50 {
lab=#net5}
N -430 -150 -310 -150 {
lab=#net1}
N -640 -190 -80 -190 {
lab=#net2}
N -80 -190 -80 -150 {
lab=#net2}
N -530 -60 -530 -20 {
lab=vss}
N -600 -60 -530 -60 {
lab=vss}
C {sky130_fd_pr/pfet_01v8.sym} -450 -230 0 0 {name=M12
L=5.0
W=1
nf=1
mult=5
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -620 -230 0 1 {name=M13
L=5.0
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {devices/ipin.sym} -630 -650 0 0 {name=p1 lab=vdd
}
C {devices/lab_pin.sym} -560 -290 0 0 {name=p2 sig_type=std_logic lab=vdd
}
C {sky130_fd_pr/pnp_05v5.sym} -450 -60 0 0 {name=Q_BR
model=pnp_05v5_W3p40L3p40
m=1
spiceprefix=X
}
C {sky130_fd_pr/pnp_05v5.sym} -620 -60 0 1 {name=Q_BL
model=pnp_05v5_W3p40L3p40
m=1
spiceprefix=X
}
C {devices/ipin.sym} -630 -610 0 0 {name=p3 lab=vss
}
C {devices/lab_pin.sym} -550 0 0 0 {name=p4 sig_type=std_logic lab=vss
}
C {sky130_fd_pr/pfet_01v8.sym} -200 -230 0 1 {name=M11
L=5.0
W=1
nf=1
mult=2
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -100 -150 0 1 {name=M6
L=5.0
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -290 -150 0 0 {name=M5
L=5.0
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} -140 -50 0 0 {name=M2
L=10.0
W=1
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} -250 -50 0 1 {name=M1
L=10.0
W=1
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 100 -230 0 0 {name=M9
L=5.0
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} 10 -230 0 1 {name=M10
L=5.0
W=1
nf=1
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 10 -150 0 1 {name=M7
L=5.0
W=1
nf=1 
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 100 -150 0 0 {name=M8
L=5.0
W=1
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} -30 -50 0 0 {name=M3
L=10.0
W=1
nf=1 
mult=4
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/nfet_01v8.sym} 100 -50 0 0 {name=M4
L=5.0
W=1
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} 210 -220 0 1 {name=p6 sig_type=std_logic lab=Vp}
C {sky130_fd_pr/pnp_05v5.sym} -240 -460 0 1 {name=Q_BL1
model=pnp_05v5_W3p40L3p40
m=1
spiceprefix=X
}
C {sky130_fd_pr/pnp_05v5.sym} -150 -460 0 0 {name=Q_BR1
model=pnp_05v5_W3p40L3p40
m=1
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -240 -580 0 1 {name=M14
L=5.0
W=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {sky130_fd_pr/pfet_01v8.sym} -150 -580 0 0 {name=M15
L=5.0
W=1
nf=1
mult=5
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {devices/lab_pin.sym} -220 -640 0 0 {name=p5 sig_type=std_logic lab=vdd
}
C {devices/lab_pin.sym} -210 -380 0 0 {name=p11 sig_type=std_logic lab=vss
}
C {devices/lab_pin.sym} -30 -570 0 1 {name=p12 sig_type=std_logic lab=Vp
}
C {devices/opin.sym} -80 -520 0 0 {name=p13 lab=Vbe2}
C {devices/opin.sym} -300 -520 0 1 {name=p14 lab=Vbe1
}
C {sky130_fd_pr/pfet_01v8.sym} -550 -500 0 0 {name=M16
L=1.2
W=10
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_01v8
spiceprefix=X
}
C {devices/ipin.sym} -620 -500 0 0 {name=p15 lab=ena}
C {devices/lab_pin.sym} -480 -530 0 1 {name=p16 sig_type=std_logic lab=vdd}
C {devices/lab_pin.sym} -480 -450 0 1 {name=p17 sig_type=std_logic lab=Vp}
C {sky130_fd_pr/res_generic_l1.sym} -640 -140 0 0 {name=R1
W=1
L=1000
model=res_generic_l1
mult=1}
C {sky130_fd_pr/res_generic_l1.sym} -500 -60 1 1 {name=R2
W=1
L=200
model=res_generic_l1
mult=1}
